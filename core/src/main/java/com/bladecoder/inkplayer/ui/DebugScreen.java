...
public class DebugScreen implements AppScreen {
	private UI ui;
	...
	public void show() {
		...
		// ------------- SPEED
		/*
		speedText = new TextField(Float.toString(((StoryScreen) ui.getScreen(Screens.SCENE_SCREEN)).getSpeed()),
				ui.getSkin());
		TextButton speedButton = new TextButton("Set Speed", ui.getSkin());
		speedButton.addListener(new ClickListener() {

			public void clicked(InputEvent event, float x, float y) {
				StoryScreen scnScr = (StoryScreen) ui.getScreen(Screens.SCENE_SCREEN);
			...
			public void clicked(InputEvent event, float x, float y) {
				...
				if(!c.isEmpty()) {
					try {
						...
						Gdx.app.error( InkApp.LOG_TAG, "Loading chapter.", e);
					}
				}
			}
		});
		...
		*/
		...
		String versionString = Config.getProperty(Config.TITLE_PROP, "title unspecified") + " v"
				...
				+ Config.getProperty(Config.BLADE_INK_VERSION_PROP, "unspecified") + "\n" + "libGdx: v"
